    ...
    def with_retry(&block)
      ...
      begin
        ...
        if retryable?(error) && attempt <= max_retries
          ...
          @before_retry.call(attempt: attempt, error: error, delay: delay) if @before_retry
          sleep(delay)
          @after_retry.call(attempt: attempt, error: error, delay: delay) if @after_retry
    ...
    def retryable?(error)
      ...
      @retryable_errors.any? do |retryable|
        if retryable.is_a?(Class)
          ...
          error.class.name == retryable.to_s
        end
      end
    end
